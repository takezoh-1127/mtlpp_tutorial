...
//
fragment float4 phongFS(VertexOut in[[stage_in]], constant UniformFS& inUniform [[buffer(3)]], texture2d<float> tex [[texture(0)]])
{
	...
	float3 specular = inUniform.specularColor * pow(max(0.0, dot(R, V)), inUniform.specularPower);
	
    //float4 out = tex.sample(quadSampler, in.texcoord) * float4(color, 1.0);
	float4 out = (tex.sample(quadSampler, in.texcoord) * float4(color, 1.0)) + float4(specular, 1.0);
	return out;
