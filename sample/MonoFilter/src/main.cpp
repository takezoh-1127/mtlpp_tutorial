...
mtlpp::Buffer g_monoFSUniformBuff;
...
// モノクロフィルターの頂点シェーダーに渡すユニフォームバッファ.
typedef struct
{
	float t;
}MonoFSUniformBuffer;
...
void update(float deltaT)
{
	...
	// モノクロフィルターのユニフォームバッファの設定.
	{
		MonoFSUniformBuffer* buffer = static_cast<MonoFSUniformBuffer*>(g_monoFSUniformBuff.GetContents());
...
void render(const Window& window)
{
	...
	// 通常のフレームバッファに描き戻す.
	{
		...
		commandBuffer.SetLabel("mono filter pass command buffer");
		...
		if(desc)
		{
			...
			encoder.SetFragmentBuffer(g_monoFSUniformBuff, 0, 0);
...
int main(int argc, const char * argv[])
{
	...
	mtlpp::Function monoVS = library.NewFunction("monoVS");
	assert(monoVS);
	
	mtlpp::Function monoFS = library.NewFunction("monoFS");
	assert(monoFS);
	...
	// モノクロフィルター描画用のレンダーパイプラインの生成.
	{
		mtlpp::RenderPipelineDescriptor desc;
			
		desc.SetVertexFunction(monoVS);
		desc.SetFragmentFunction(monoFS);
	...
	// ユニフォームバッファの生成.
	{
		g_monoFSUniformBuff = g_device.NewBuffer(sizeof(MonoFSUniformBuffer), mtlpp::ResourceOptions::StorageModeShared);
		assert(g_monoFSUniformBuff);
	}
