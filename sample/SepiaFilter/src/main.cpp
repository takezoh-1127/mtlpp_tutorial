...
//  SepiaFilter
//
//  Created by TAKEZOH on 2020/03/26.
...
mtlpp::Buffer g_sepiaFSUniformBuff;
...
// phongシェーダーの頂点シェーダーに渡すユニフォームバッファ.
typedef struct
{
...
// セピアフィルターの頂点シェーダーに渡すユニフォームバッファ.
typedef struct
{
	float t;
}SepiaFSUniformBuffer;
...
void update(float deltaT)
{
	...
	// セピアフィルターのユニフォームバッファの設定.
	{
		SepiaFSUniformBuffer* buffer = static_cast<SepiaFSUniformBuffer*>(g_sepiaFSUniformBuff.GetContents());
...
void render(const Window& window)
{
	// 通常シーンの描画.
	{
		auto commandBuffer = g_commandQueue.CommandBuffer();
	...
	// 通常のフレームバッファに描き戻す.
	{
		...
		commandBuffer.SetLabel("sepia filter pass command buffer");
				
		auto desc = window.GetRenderPassDescriptor();
		...
		if(desc)
		{
			...
			encoder.SetFragmentBuffer(g_sepiaFSUniformBuff, 0, 0);
...
int main(int argc, const char * argv[])
{
	...
	mtlpp::Function sepiaVS = library.NewFunction("sepiaVS");
	assert(sepiaVS);
	
	mtlpp::Function sepiaFS = library.NewFunction("sepiaFS");
	assert(sepiaFS);
	...
	// セピアフィルター描画用のレンダーパイプラインの生成.
	{
		mtlpp::RenderPipelineDescriptor desc;
			
		desc.SetVertexFunction(sepiaVS);
		desc.SetFragmentFunction(sepiaFS);
	...
	// ユニフォームバッファの生成.
	{
		g_sepiaFSUniformBuff = g_device.NewBuffer(sizeof(SepiaFSUniformBuffer), mtlpp::ResourceOptions::StorageModeShared);
		assert(g_sepiaFSUniformBuff);
	}
